...
class plgContentJw_allvideos extends JPlugin {
  ...
  // JoomlaWorks reference parameters
	...
	var $plg_copyrights_start		= "\n\n<!-- JoomlaWorks \"AllVideos\" Plugin (v4.5.1) starts here -->\n";
	var $plg_copyrights_end			= "\n<!-- JoomlaWorks \"AllVideos\" Plugin (v4.5.1) ends here -->\n\n";
	...
	function renderAllVideos(&$row, &$params, $page = 0) {
		...
		if (JRequest::getCmd('format')=='html' || JRequest::getCmd('format')=='') {
			...
			if($jwPlayerLoading=='local'){
				if ($gzipScripts) {
					$document->addScript($pluginLivePath.'/includes/js/jwp_all.js.php?v=4.5.1');
				} else {
					$document->addScript($pluginLivePath.'/includes/js/behaviour.js?v=4.5.1');
					$document->addScript($pluginLivePath.'/includes/js/jwplayer/jwplayer.js?v=4.5.1');
					$document->addScript($pluginLivePath.'/includes/js/wmvplayer/silverlight.js?v=4.5.1');
					$document->addScript($pluginLivePath.'/includes/js/wmvplayer/wmvplayer.js?v=4.5.1');
					$document->addScript($pluginLivePath.'/includes/js/quicktimeplayer/AC_QuickTime.js?v=4.5.1');
				}
				$document->addScriptDeclaration('jwplayer.key="'.$jwPlayerAPIKey.'";');
			} else {
				if ($gzipScripts) {
					$document->addScript($pluginLivePath.'/includes/js/jwp_swq.js.php?v=4.5.1');
				} else {
					$document->addScript($pluginLivePath.'/includes/js/behaviour.js?v=4.5.1');
					$document->addScript($pluginLivePath.'/includes/js/wmvplayer/silverlight.js?v=4.5.1');
					$document->addScript($pluginLivePath.'/includes/js/wmvplayer/wmvplayer.js?v=4.5.1');
